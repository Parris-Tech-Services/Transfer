...
try {
  ...
  for (const name of expected) {
    ...
    const h1 = page.locator("main h1"),
      title = (await h1.count()) ? await h1.textContent() : null;
    if (title?.trim() !== name)
      throw new Error(
        `Navigation to ${name} failed; still on ${title}; renderer errors: ${errors.join(" | ")}`,
      );
    await page.screenshot({
      path: path.join(out, `${name.toLowerCase().replaceAll(" ", "-")}.png`),
      fullPage: false,
    });
  }
  ...
  if (process.env.LIFEBOAT_LIVE === "1" && !dashboard.latestInventory) {
    ...
    await page.waitForFunction(() => !document.body.innerText.includes("Inventory running"), null, {
      timeout: 30 * 60_000,
    });
    ...
    assert(
      after.inventory.logs.some((x) => x.done),
      "live inventory did not produce completion logs",
    );
  }
  ...
  if (process.env.LIFEBOAT_LIVE === "1") {
    ...
    assert(
      discovered.gmail.runs.length > gmail.gmail.runs.length,
      "Gmail dry run did not persist a run",
    );
  }
  await page.getByRole("button", { name: "Security", exact: true }).click();
  assert.equal(
    (await page.getByRole("button", { name: /Revoke|remove/i }).count()) > 0,
    true,
  ...
  console.log(
    JSON.stringify(
      {
        ok: true,
        live: process.env.LIFEBOAT_LIVE === "1",
        screens: expected.length,
        inventory: !!dashboard.latestInventory,
        gmailRuns: dashboard.gmail.runs.length,
      },
      null,
      2,
    ),
